...
# Assignment: 3
# Due Date: 11/6/23
# Description: This is an implementation of a static array data structure
# using a queue ADT (first in first out).
...
class Queue:
    ...
    def enqueue(self, value: object) -> None:
        """
        Add an element to the back of the queue. This is the end of the
        static array.
        ...
        # Check back end value for circular buffer, reindex to zero?
    ...
    def dequeue(self) -> object:
        """
        Remove the item at the front of the queue. This corresponds to the
        ...
        # value to be dequeued and returned
        value = self._sa[self._front]
        # Get the new index via increment, just in case front needs to circle.
        index = self._increment(self._front)
        self._front = index
        self._current_size -= 1
        return value
    ...
    def front(self) -> object:
        """
        View the front item in the queue, aka, the next item to be dequeued.
        Returns: Queue object
        """
        if self.is_empty():
            raise QueueException
        # Return value of front index
    ...
    def _double_queue(self) -> None:
        """
        Double the size of the underlying array, then copies the old values
        over to the new, reindexing as it goes.
        """
        # Double old capacity
        new_array = StaticArray(self._current_size*2)

        index = self._front
        # Copy over new values, reindex to normalize indexing from 0.
        for i in range(self._current_size):
            new_array[i] = self._sa[index]
            index = self._increment(index)
        # Set old array to new, resent front and back.
        self._sa = new_array
        self._front = 0
